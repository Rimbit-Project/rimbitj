...
/**
 * A collection of various utility methods that are helpful for working with the Rimbit protocol.
 * To enable debug logging from the library, run with -Drimbitj.logging=true on your command line.
 */
public class Utils {
    ...
    /** The string that prefixes all text messages signed using Rimbit keys. */
    public static final String Rimbit_SIGNED_MESSAGE_HEADER = "Rimbit Signed Message:\n";
    public static final byte[] Rimbit_SIGNED_MESSAGE_HEADER_BYTES = Rimbit_SIGNED_MESSAGE_HEADER.getBytes(Charsets.UTF_8);

    // TODO: Replace this nanocoins business with something better.

    /**
     * How many "nanocoins" there are in a Rimbit.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Rimbit.
    ...
    /**
     * How many "nanocoins" there are in 0.01 Rimbits.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Rimbit.
     * The term nanocoin is very misleading, though, because there are only 100 million
     * of them in a coin (whereas one would expect 1 billion).
    ...
    public static BigInteger toNanoCoins(String coins) {
        ...
        if (bigint.compareTo(NetworkParameters.MAX_MONEY) > 0)
            throw new ArithmeticException("Amount larger than the total quantity of Rimbits possible specified.");
        return bigint;
    }
     ...
     * standard procedure in Rimbit. The resulting hash is in big endian form.
     */
     ...
     */
    public static String rimbitValueToFriendlyString(BigInteger value) {
     ...
     */
    public static String rimbitValueToPlainString(BigInteger value) {
        if (value == null) {
            throw new IllegalArgumentException("Value cannot be null");
        }
                
        BigDecimal valueInRBT = new BigDecimal(value).divide(new BigDecimal(Utils.COIN));
        return valueInRBT.toPlainString();
    }
     ...
     *
     * <p>Rimbit only uses this "compact" format for encoding difficulty targets, which are unsigned 256bit quantities.
     ...
     *
     * <tt><p>[24] "Rimbit Signed Message:\n" [message.length as a varint] message</p></tt>
     */
    public static byte[] formatMessageForSigning(String message) {
        try {
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            bos.write(Rimbit_SIGNED_MESSAGE_HEADER_BYTES.length);
            bos.write(Rimbit_SIGNED_MESSAGE_HEADER_BYTES);
