...
import com.rimbit.rimbit.crypto.KeyCrypter;
import com.rimbit.rimbit.crypto.KeyCrypterException;
import com.rimbit.rimbit.crypto.KeyCrypterScrypt;
import com.rimbit.rimbit.script.Script;
import com.rimbit.rimbit.script.ScriptBuilder;
import com.rimbit.rimbit.script.ScriptChunk;
import com.rimbit.rimbit.store.UnreadableWalletException;
import com.rimbit.rimbit.store.WalletProtobufSerializer;
import com.rimbit.rimbit.utils.ListenerRegistration;
import com.rimbit.rimbit.utils.Threading;
import com.rimbit.rimbit.wallet.*;
import com.rimbit.rimbit.wallet.WalletTransaction.Pool;
...
import org.rimbitj.wallet.Protos.Wallet.EncryptionType;
...
import static com.rimbit.rimbit.core.Utils.rimbitValueToFriendlyString;
import static com.rimbit.rimbit.core.Utils.rimbitValueToPlainString;
 ...
 *
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of rimbitj,
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.rimbit.rimbit.wallet.WalletFiles.Listener)}
...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
    ...
    public void receivePending(Transaction tx, @Nullable List<Transaction> dependencies, boolean overrideIsRelevant) throws VerificationException {
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s RBT from our own wallet," +
                        " and sends us %s RBT", tx.getHashAsString(), Utils.rimbitValueToFriendlyString(valueSentFromMe),
                        Utils.rimbitValueToFriendlyString(valueSentToMe)));
            }
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
        ...
        log.info("Received tx{} for {} RBT: {} [{}] in block {}", sideChain ? " on a side chain" : "",
                rimbitValueToFriendlyString(valueDifference), tx.getHashAsString(), relativityOffset,
        ...
        if (!insideReorg && bestChain) {
            ...
            log.info("Balance is now: " + rimbitValueToFriendlyString(newBalance));
            if (!wasPending) {
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getHash(), new String(Hex.encode(tx.unsafeRimbitSerialize())));
                    Transaction other = input.getConnectedOutput().getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getHash(), new String(Hex.encode(tx.unsafeRimbitSerialize())));
                }
    ...
    public static class SendResult {
        /** The Rimbit transaction message that moves the money. */
    ...
    public static class SendRequest {
         ...
         * (its value is ignored and set to {@link com.rimbit.rimbit.core.Wallet#getBalance()} - the fees required
         ...
         * is how mining is incentivized in later years of the Rimbit system when inflation drops. It also provides
         ...
         *
         * <p>Once {@link Wallet#completeTx(com.rimbit.rimbit.core.Wallet.SendRequest)} is called, this is set to the
         ...
         * 
         * <p>Rimbit: This is zero even though we add a required fee. This is an extra fee on top that we don't want.</p>
         */
        ...
        /**
         * If not null, the {@link com.rimbit.rimbit.wallet.CoinSelector} to use instead of the wallets default. Coin selectors are
     ...
     *
     * @param address       The Rimbit address to send the money to.
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            if (bestChangeOutput != null) {
                ...
                log.info("  with {} coins change", rimbitValueToFriendlyString(bestChangeOutput.getValue()));
            }
            final BigInteger calculatedFee = totalInput.subtract(totalOutput);
            if (calculatedFee.compareTo(BigInteger.ZERO) > 0) {
                log.info("  with a fee of {}", rimbitValueToFriendlyString(calculatedFee));
            }
            ...
            // Check size.
            int size = req.tx.rimbitSerialize().length;
    ...
    private boolean adjustOutputDownwardsForFee(Transaction tx, CoinSelection coinSelection, BigInteger baseFee, BigInteger feePerKb) {
        ...
        int size = tx.rimbitSerialize().length;
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.rimbit.rimbit.wallet.WalletFiles.Listener)}
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s RBT (available: %s RBT) in:%n",
                    rimbitValueToPlainString(estimatedBalance), rimbitValueToPlainString(availableBalance)));
    ...
    private void toStringHelper(StringBuilder builder, Map<Sha256Hash, Transaction> transactionMap,
        ...
        for (Transaction tx : txns) {
            try {
                ...
                builder.append(Utils.rimbitValueToFriendlyString(tx.getValueSentFromMe(this)));
                builder.append(" and receives ");
                builder.append(Utils.rimbitValueToFriendlyString(tx.getValueSentToMe(this)));
                builder.append(", total value ");
                builder.append(Utils.rimbitValueToFriendlyString(tx.getValue(this)));
                builder.append(".\n");
            } catch (ScriptException e) {
    ...
    public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks, List<StoredBlock> newBlocks) throws VerificationException {
        ...
        try {
            ...
            log.info("post-reorg balance is {}", Utils.rimbitValueToFriendlyString(balance));
     ...
     * of {@link com.rimbit.rimbit.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(com.rimbit.rimbit.crypto.KeyCrypter,
     ...
     * {@link com.rimbit.rimbit.crypto.KeyCrypterScrypt}.
     *
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all rimbit controlled by that private key is lost forever).
    ...
    /**
     * <p>Convenience wrapper around {@link Wallet#addNewEncryptedKey(com.rimbit.rimbit.crypto.KeyCrypter,
            ...
            for (int i = 0; i < tx.getOutputs().size(); i++) {
                ...
                try {
                    if (isTxOutputBloomFilterable(out)) {
                        ...
                        filter.insert(outPoint.rimbitSerialize());
                    }
                } catch (ScriptException e) {
                    throw new RuntimeException(e); // If it is ours, we parsed the script correctly, so this shouldn't happen
                }
     ...
     * you can use {@link com.rimbit.rimbit.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            while (true) {
                ...
                // If change is < 0.001 RBT, we will need to discard the change
                ...
                if (change.compareTo(BigInteger.ZERO) > 0) {
                    ...
                    size += changeOutput.rimbitSerialize().length + VarInt.sizeOf(req.tx.getOutputs().size()) - VarInt.sizeOf(req.tx.getOutputs().size() - 1);
                ...
                size += req.tx.rimbitSerialize().length;
            ...
            if (selection2 == null && selection1 == null) {
                ...
                log.warn("Insufficient value in wallet for send: needed {} more", rimbitValueToFriendlyString(valueMissing));
     ...
     * sending transactions to the Rimbit network by default. Some sendCoins methods let you specify a broadcaster
     ...
     */
    public void setTransactionBroadcaster(@Nullable com.rimbit.rimbit.core.TransactionBroadcaster broadcaster) {
    ...
    private Transaction rekeyOneBatch(long keyRotationTimestamp, final TransactionBroadcaster broadcaster) {
        ...
        try {
            ...
            checkState(rekeyTx.rimbitSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            commitTx(rekeyTx);
     ...
     * {@link com.rimbit.rimbit.core.PeerFilterProvider} interface and generally should not be used directly by apps.
