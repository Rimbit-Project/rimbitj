...
import com.rimbit.rimbit.net.AbstractTimeoutHandler;
import com.rimbit.rimbit.net.MessageWriteTarget;
import com.rimbit.rimbit.net.StreamParser;
...
public abstract class PeerSocketHandler extends AbstractTimeoutHandler implements StreamParser {
    ...
    private RimbitSerializer.RimbitPacketHeader header;
    ...
    public PeerSocketHandler(NetworkParameters params, InetSocketAddress remoteIp) {
        serializer = new RimbitSerializer(checkNotNull(params));
    ...
    public int receiveBytes(ByteBuffer buff) {
        checkArgument(buff.position() == 0 &&
                buff.capacity() >= RimbitSerializer.RimbitPacketHeader.HEADER_LENGTH + 4);
        try {
     ...
     * called automatically by {@link com.rimbit.rimbit.net.NioClient} or
     * {@link com.rimbit.rimbit.net.NioClientManager} once the socket finishes initialization.
     */
    @Override
