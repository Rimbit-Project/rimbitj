...
public class FilteredBlock extends Message {
    ...
    public void rimbitSerializeToStream(OutputStream stream) throws IOException {
        if (header.transactions == null)
            header.rimbitSerializeToStream(stream);
        else
            header.cloneAsHeader().rimbitSerializeToStream(stream);
        merkleTree.rimbitSerializeToStream(stream);
    }
