...
import com.rimbit.rimbit.crypto.TransactionSignature;
 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the Rimbit C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of Rimbit and have many powerful features. Read
 ...
 *
 * <p>All Rimbit transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage implements Serializable {
     ...
     */
    public static final BigInteger MIN_NONDUST_OUTPUT = BigInteger.valueOf(1000);

    // These are serialized in both rimbit and java serialization.
     ...
     * @param msg Rimbit protocol formatted byte array containing message content.
    ...
    /**
     * Used by RimbitSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of Rimbit share. In 2011 the value of a coinbase
    ...
    public String toString(@Nullable AbstractBlockChain chain) {
        ...
        for (TransactionOutput out : outputs) {
            ...
            try {
                ...
                s.append(" ");
                s.append(rimbitValueToFriendlyString(out.getValue()));
                s.append(" RBT");
    ...
    public void clearInputs() {
        ...
        this.length = this.rimbitSerialize().length;
    }
     ...
     * and requires external synchronization. Please refer to general documentation on Rimbit scripting and contracts
    ...
    /**
     * Same as {@link #addSignedInput(TransactionOutPoint, com.rimbit.rimbit.script.Script, ECKey, com.rimbit.rimbit.core.Transaction.SigHash, boolean)}
     ...
     * a wrapper around calling {@link Transaction#hashForSignature(int, byte[], com.rimbit.rimbit.core.Transaction.SigHash, boolean)}
     ...
     * features of Rimbit are developed.</p>
     *
    ...
    public synchronized Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        //
        //   https://en.rimbit.it/wiki/Contracts
        ...
        try {
            ...
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in Rimbit 0.1.
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // Rimbit implementation. The reference client also contains a bug in handling this case:
    ...
    @Override
    protected void rimbitSerializeToStream(OutputStream stream) throws IOException {
        uint32ToByteStreamLE(version, stream);
        uint32ToByteStreamLE(time, stream);
        stream.write(new VarInt(inputs.size()).encode());
        for (TransactionInput in : inputs)
            in.rimbitSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.rimbitSerialize(stream);
     ...
     * since Rimbit 0.8+ a transaction that did not end its lock period (non final) is considered to be non
     ...
     * This is useful in certain types of <a href="http://en.rimbit.it/wiki/Contracts">contracts</a>, such as
