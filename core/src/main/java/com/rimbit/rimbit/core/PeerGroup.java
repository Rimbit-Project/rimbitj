...
import com.rimbit.rimbit.net.ClientConnectionManager;
import com.rimbit.rimbit.net.FilterMerger;
import com.rimbit.rimbit.net.NioClientManager;
...
import com.rimbit.rimbit.utils.ExponentialBackoff;
...
public class PeerGroup extends AbstractExecutionThreadService implements TransactionBroadcaster {
    ...
    // Minimum protocol version we will allow ourselves to connect to: Do not require bloom filtering as it doesn't exist in Rimbit
    ...
    private ExponentialBackoff.Params peerBackoffParams = new ExponentialBackoff.Params(1000, 5.0f, 10 * 60 * 1000);
    // Tracks failures globally in case of a network failure.
    private ExponentialBackoff groupBackoff = new ExponentialBackoff(new ExponentialBackoff.Params(10, 1.3f, 5 * 1000));
     ...
     * <a href="https://github.com/rimbit/bips/blob/master/bip-0014.mediawiki">BIP 14</a>.
     *
    ...
    protected void connectToAnyPeer() throws PeerDiscoveryException {
        ...
        try {
            ...
            retryTime = (backoff != null) ? backoff.getRetryTime() : 10000;
     ...
     *         Use {@link com.rimbit.rimbit.core.Peer#getConnectionOpenFuture()} if you
     ...
     * peers. By using this with {@link com.rimbit.rimbit.core.PeerGroup#getMaxConnections()} you can wait until the
     ...
     * {@link com.rimbit.rimbit.core.PeerGroup#getMaxConnections()} returns is used.
     */
    ...
    /**
     * See {@link com.rimbit.rimbit.core.PeerGroup#getMinBroadcastConnections()}.
     */
     ...
     * someone and want to know that it's valid. It's a bit of a weird hack because the current version of the Rimbit
     ...
     * times are available via {@link com.rimbit.rimbit.core.Peer#getLastPingTime()} but it increases load on the
     ...
     * using {@link com.rimbit.rimbit.core.Peer#ping()}.
     */
