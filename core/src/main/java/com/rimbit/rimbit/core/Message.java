...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Rimbit proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param protocolVersion Rimbit protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = rimbitSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Rimbit protocol.
    ...
    /**
     * Should only used by RimbitSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by RimbitSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeRimbitSerialize()}, which is safe to mutate.
     ...
     */
    public byte[] rimbitSerialize() {
        byte[] bytes = unsafeRimbitSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the rimbit wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeRimbitSerialize() {
        ...
        try {
            rimbitSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to rimbitSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the rimbit wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void rimbitSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use rimbitSerialize().
     */
    void rimbitSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented rimbitSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so RimbitSerializer can avoid 2 instanceof checks + a casting.
     */
