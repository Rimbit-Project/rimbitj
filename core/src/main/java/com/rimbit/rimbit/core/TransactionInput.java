...
public class TransactionInput extends ChildMessage implements Serializable {
    ...
    protected void rimbitSerializeToStream(OutputStream stream) throws IOException {
        outpoint.rimbitSerialize(stream);
     ...
     * "from address" is not well defined in Rimbit and you should not assume that senders of a transaction can
     ...
     * in nodes memory pools if the existing version is time locked. See the Contracts page on the Rimbit wiki for
