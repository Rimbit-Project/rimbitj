...
/**
 * <p>Provides a standard implementation of a Rimbit URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code rimbit:<address>}</li>
 * <li>{@code rimbit:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://github.com/rimbit/bips/blob/master/bip-0021.mediawiki">BIP 0021</a>
 */
public class RimbitURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(RimbitURI.class);
    ...
    public static final String Rimbit_SCHEME = "rimbit";
    ...
    /**
     * Constructs a new RimbitURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws RimbitURIParseException if the URI is not syntactically or semantically valid.
     */
    public RimbitURI(String uri) throws RimbitURIParseException {
        this(null, uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid Rimbit URI.
     *
     ...
     *
     * @throws RimbitURIParseException If the input fails Rimbit URI syntax and semantic checks.
     */
    public RimbitURI(@Nullable NetworkParameters params, String input) throws RimbitURIParseException {
        checkNotNull(input);
        ...
        try {
            ...
            throw new RimbitURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  rimbit:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form rimbit://address?....
        // We support both until Ben fixes his code.
        
        // Remove the rimbit scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : rimbit:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("rimbit://")) {
            schemeSpecificPart = input.substring("rimbit://".length());
        } else if (input.startsWith("rimbit:")) {
            schemeSpecificPart = input.substring("rimbit:".length());
        } else {
            throw new RimbitURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0)
            throw new RimbitURIParseException("No data found after the rimbit: prefix");
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new RimbitURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a Rimbit address for this network
            try {
                Address address = new Address(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new RimbitURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new RimbitURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws RimbitURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new RimbitURIParseException("Malformed Rimbit URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new RimbitURIParseException("Malformed Rimbit URI - empty name '" +
                        nameValuePairToken + "'");
     ...
     */
    private void putWithValidation(String key, Object value) throws RimbitURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new RimbitURIParseException(String.format("'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The Rimbit Address from the URI, if one was present. It's possible to have Rimbit URI's with no address if a
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("RimbitURI[");
        boolean first = true;
    ...
    public static String convertToRimbitURI(Address address, BigInteger amount, String label, String message) {
        return convertToRimbitURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Rimbit URI builder using known good fields.
     * 
     * @param address The Rimbit address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Rimbit URI
     */
    public static String convertToRimbitURI(String address, @Nullable BigInteger amount, @Nullable String label,
        ...
        builder.append(Rimbit_SCHEME).append(":").append(address);
        
        boolean questionMarkHasBeenOutput = false;
        
        if (amount != null) {
            builder.append(QUESTION_MARK_SEPARATOR).append(FIELD_AMOUNT).append("=");
            builder.append(Utils.rimbitValueToPlainString(amount));
