...
package com.rimbit.rimbit.script;
...
import static com.rimbit.rimbit.script.ScriptOpCodes.*;
import static com.rimbit.rimbit.core.Utils.bytesToHexString;
 ...
 *
 * <p>Rimbit transactions don't specify what they do directly. Instead <a href="https://en.rimbit.it/wiki/Script">a
...
public class Script {
     ...
     * The concept of a "from address" isn't well defined in Rimbit and you should not assume the sender of a
     ...
     * logic is defined by <a href="https://github.com/rimbit/bips/blob/master/bip-0016.mediawiki">BIP 16</a>.</p>
     *
     * <p>rimbitj does not support creation of P2SH transactions today. The goal of P2SH is to allow short addresses
    ...
    private static void executeCheckSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        try {
            TransactionSignature sig  = TransactionSignature.decodeFromRimbit(sigBytes, false);
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        while (sigs.size() > 0) {
            ...
            try {
                TransactionSignature sig = TransactionSignature.decodeFromRimbit(sigs.getFirst(), false);
                Sha256Hash hash = txContainingThis.hashForSignature(index, connectedScript, (byte) sig.sighashFlags);
    ...
    public void correctlySpends(Transaction txContainingThis, long scriptSigIndex, Script scriptPubKey,
        ...
        try {
            txContainingThis = new Transaction(txContainingThis.getParams(), txContainingThis.rimbitSerialize());
