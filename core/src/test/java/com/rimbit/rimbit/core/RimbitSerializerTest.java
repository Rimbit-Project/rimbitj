...
public class RimbitSerializerTest {
    ...
    public void testAddr() throws Exception {
        RimbitSerializer bs = new RimbitSerializer(MainNetParams.get());
        // the actual data from https://en.rimbit.it/wiki/Protocol_specification#addr
    ...
    public void testLazyParsing()  throws Exception {
        RimbitSerializer bs = new RimbitSerializer(MainNetParams.get(), true, false);
    ...
    private void testCachedParsing(boolean lazy)  throws Exception {
        RimbitSerializer bs = new RimbitSerializer(MainNetParams.get(), lazy, true);
    ...
    @Test
    public void testRimbitPacketHeader() {
        try {
            new RimbitSerializer.RimbitPacketHeader(ByteBuffer.wrap(new byte[]{0}));
            fail();
        ...
        try {
            new RimbitSerializer.RimbitPacketHeader(ByteBuffer.wrap(wrongMessageLength));
            fail();
    ...
    public void testSeekPastMagicBytes() {
        ...
        try {
            new RimbitSerializer(MainNetParams.get()).seekPastMagicBytes(ByteBuffer.wrap(brokenMessage));
            fail();
        } catch (BufferUnderflowException e) {
            // expected
        }
