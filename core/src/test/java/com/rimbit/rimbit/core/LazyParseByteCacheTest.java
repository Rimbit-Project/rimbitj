...
import com.rimbit.rimbit.params.UnitTestParams;
import com.rimbit.rimbit.store.BlockStore;
import com.rimbit.rimbit.store.MemoryBlockStore;
...
import static com.rimbit.rimbit.utils.TestUtils.createFakeBlock;
import static com.rimbit.rimbit.utils.TestUtils.createFakeTx;
...
public class LazyParseByteCacheTest {
    ...
    public void setUp() throws Exception {
        ...
        RimbitSerializer bs = new RimbitSerializer(unitTestParams);
        
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        bs.serialize(tx1, bos);
        tx1BytesWithHeader = bos.toByteArray();
        tx1Bytes = tx1.rimbitSerialize();
        
        bos.reset();
        bs.serialize(tx2, bos);
        tx2BytesWithHeader = bos.toByteArray();
        tx2Bytes = tx2.rimbitSerialize();
        
        bos.reset();
        bs.serialize(b1, bos);
        b1BytesWithHeader = bos.toByteArray();
        b1Bytes = b1.rimbitSerialize();
    }
    ...
    public void testBlock(byte[] blockBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
        ...
        //message structure.
        RimbitSerializer bsRef = new RimbitSerializer(unitTestParams, false, false);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        
        RimbitSerializer bs = new RimbitSerializer(unitTestParams, lazy, retain);
        Block b1;
        Block bRef;
        b1 = (Block) bs.deserialize(ByteBuffer.wrap(blockBytes));
        bRef = (Block) bsRef.deserialize(ByteBuffer.wrap(blockBytes));
        
        //verify our reference RimbitSerializer produces matching byte array.
        bos.reset();
    ...
    public void testTransaction(NetworkParameters params, byte[] txBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
        ...
        //reference serializer to produce comparison serialization output after changes to
        //message structure.
        RimbitSerializer bsRef = new RimbitSerializer(params, false, false);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        
        RimbitSerializer bs = new RimbitSerializer(params, lazy, retain);
        Transaction t1;
        Transaction tRef;
    ...
    private void serDeser(RimbitSerializer bs, Message message, byte[] sourceBytes, byte[] containedBytes, byte[] containingBytes) throws Exception {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
