...
package com.rimbit.rimbit.tools;
...
import com.rimbit.rimbit.store.*;
...
/**
 * A command line tool for manipulating wallets and working with Rimbit.
 */
public class WalletTool {
    ...
    private static org.rimbit.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    public static class Condition {
        ...
        public boolean matchRimbits(BigInteger comparison) {
            try {
    ...
    private static void send(List<String> outputs, BigInteger fee, String lockTimeStr, boolean allowUnconfirmed) throws VerificationException {
        try {
            ...
            System.err.println("Insufficient funds: have " + Utils.rimbitValueToFriendlyString(wallet.getBalance()));
        }
    }

    private static void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith("rimbit")) {
            try {
                ListenableFuture<PaymentSession> future;
                if (location.startsWith("http")) {
                    future = PaymentSession.createFromUrl(location, verifyPki);
                } else {
                    RimbitURI paymentRequestURI = new RimbitURI(location);
                    future = PaymentSession.createFromRimbitUri(paymentRequestURI, verifyPki);
                }
                ...
                System.exit(1);
            } catch (RimbitURIParseException e) {
                System.err.println("Invalid rimbit uri: " + e.getMessage());
                System.exit(1);
            } catch (InterruptedException e) {
                // Ignore.
            } catch (ExecutionException e) {
            ...
            try {
                paymentRequest = org.rimbit.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchRimbits(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
                wallet.addEventListener(new AbstractWalletEventListener() {
                    ...
                    public synchronized void onChange() {
                        ...
                        if (condition.matchRimbits(balance)) {
                            System.out.println(Utils.rimbitValueToFriendlyString(balance));
                            latch.countDown();
                        }
                    }
                });
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a Rimbit address of the right network parameters.");
                return;
            }
        }
        if (key == null) {
